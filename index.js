...
function dialog(title, msg){
  if ('object' === typeof msg) return new Dialog(extend({ message: title }, msg || {}));
  if ('string' === typeof msg) return new Dialog({ title: title, message: msg });
  else                         return new Dialog({ message: title });
};
...
function Dialog(options) {
  ...
  Emitter.call(this);
  options = options || {};
  this.template = options.template || require('./template.html');
...
Dialog.prototype.render = function(options){
  ...
  var closeLink = query('.close', el);
  if (closeLink) {
    events.bind(closeLink, 'click', function (ev) {
      ev.preventDefault();
      self.emit('close');
      self.hide();
    });
